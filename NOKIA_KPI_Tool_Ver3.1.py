# -*- coding: utf-8 -*-
"""
Created on Tue Oct 26 15:18:21 2021
...
def update_cell_value_ms(cell_val): #minus sector
    ...
    if orig_return[0:3]=='GSM':
        return 'GSM'
    sectors_allowed = ['L1900', 'L2500', 'U1900', 'U2100', 'L2100']
...
def generate_umts_remarks(site_list,fdataset,col_dict_df):
    ...
    for site in site_list:
        ...
        for row in fdataset.iloc:
            if row[col_dict_df["SITE"]] == site:
                if row[col_dict_df["Cell Availability (Blocked by user + Auto Downtime)"]] == 0:
                    val = update_cell_value(row[col_dict_df["CELL"]])
                    main_output[site].append(val)
        if len(main_output[site]) > 0:
    ...
    for site in site_list:
        ...
        for row in fdataset.iloc:
            if row[col_dict_df["SITE"]] == site:
                if row[col_dict_df["Cell Availability (Blocked by user + Auto Downtime)"]] > 0 and row[col_dict_df["Cell Availability (Blocked by user + Auto Downtime)"]] < 98:
                    val = update_cell_value(row[col_dict_df["CELL"]])
                    main_output[site].append(val)
        if len(main_output[site]) > 0:
